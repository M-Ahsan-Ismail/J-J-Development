...
class PoRequest(models.Model):
    ...
    def create_rfq(self):
        self.ensure_one()
        if self.state == 'req_created':
            raise UserError(_("RFQs have already been created for this request."))
        ...
        # Clear existing purchase orders to avoid duplicates
        self.purchase_order_ids = [(5, 0, 0)]

        # Debug vendor IDs
        for rec in self.po_request_lines:
            for vendor in rec.vendor_ids:
                print(f"Vendor ID: {vendor.id}, Name: {vendor.name}")

        # Group lines by vendors and merge quantities for the same product
        vendor_lines = {}
        for rec in self.po_request_lines:
            for vendor in rec.vendor_ids:
                if vendor.id not in vendor_lines:
                    vendor_lines[vendor.id] = {}
                product_key = rec.product_id.id
                if product_key not in vendor_lines[vendor.id]:
                    vendor_lines[vendor.id][product_key] = {
                        'line': rec,
                        'quantity': rec.request_quantity,
                        'discount': rec.discount or 0.0,
                    }
                else:
                    vendor_lines[vendor.id][product_key]['quantity'] += rec.request_quantity

        # Debug the grouped vendor lines
        print("Vendor lines after grouping:", vendor_lines)

        # Create purchase orders for each vendor
        purchase_orders = []
        for vendor_id, product_dict in vendor_lines.items():
            order_lines = [
                (0, 0, {
                    'product_id': data['line'].product_id.id,
                    'product_qty': data['quantity'],
                    'name': data['line'].product_id.name,
